...
struct MetadataBuffers {
    ...
    color_profile_cicp: MetadataBlock<CicpProfile>,
    color_profile_icc: MetadataBlock<Vec<u8>>,
...
impl<'stream> ImageReader<'stream> {
    ...
    fn fill_header_metadata_if_any(&mut self) {
        ...
        get_if_needed(
            ...
            <dyn ImageDecoder + '_>::color_profile_to_cicp,
            &mut self.metadata_buffers.color_profile_cicp,
            first_meta_retrieved,
        );
        ...
        get_if_needed(
            &mut self.inner,
            &format_attrs.color_profile,
            <dyn ImageDecoder + '_>::color_profile_to_icc,
            &mut self.metadata_buffers.color_profile_icc,
            first_meta_retrieved,
        );
...
impl<'lt> DecodedImageMetadata<'lt> {
    ...
    /// Get the color profile of the previous image as CICP is possible
    pub fn color_profile_cicp(&mut self) -> ImageResult<Option<CicpProfile>> {
        Self::access_block_with(
            &mut self.metadata_buffers.color_profile_cicp,
    ...
    /// Get the color profile of the previous image as ICC if possible
    pub fn color_profile_icc(&mut self) -> ImageResult<Option<Vec<u8>>> {
        Self::access_block_with(
            &mut self.metadata_buffers.color_profile_icc,
            self.inner.format_attributes().color_profile,
            self.inner,
    ...
    fn apply_metdata(
        ...
        let icc = match self.color_profile_icc() {
            Ok(x) => x,
        ...
        let cicp = match self.color_profile_cicp() {
            Ok(x) => x,
            Err(ImageError::Unsupported(_)) => None,
            Err(e) => {
                return Err(e);
        ...
        let cicp = if let Some(cicp) = cicp {
            // CICP values, if provided, should be consistent with ICC; arbitrarily prefer them
            // in case there is a CMS issue or slight inconsistency
            Some(cicp.cicp)
        } else if let Some(icc) = icc {
            crate::metadata::cms_provider().parse_icc(&icc)
        } else {
            None
        };
